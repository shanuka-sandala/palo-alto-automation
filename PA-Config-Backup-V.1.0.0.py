...
__blog__ = "https://the-debugging-diaries.com"
...
class FirewallBackupTool:
    ...
    def create_widgets(self):
        ...
        self.ip_entry.grid(column=0, row=1, padx=5, pady=5, columnspan=2)

        # Buttons
        self.button_frame = ttk.Frame(self.root)
        self.button_frame.grid(column=0, row=4, padx=5, pady=5, columnspan=2, sticky=tk.EW)
        ...
        self.log_text.grid(column=0, row=5, padx=5, pady=5, columnspan=2)
...
Password: {"*" * len(PASSWORD)}"""
    ...
    def backup_firewall(self, ip):
        try:
            # Get API key first
            api_key = self.get_api_key(ip)
            if not api_key:
    ...
    def start_backup(self):
        ...
        for ip in ips:
            thread = threading.Thread(target=self.backup_firewall, args=(ip,))
            thread.daemon = True
            thread.start()
